...
def T(elm, currentTime):
    if elm[2] - (currentTime + elm[1]) >= 0:
        return (elm[2] - (currentTime + elm[1]))*elm[3]
    else:
        return ((currentTime + elm[1]) - elm[2])*elm[4]
    ...
    if len(lst) != 0:
        for elm in range(0, len(lst)):
            ...
            if identifier in D.keys():
                res = T(lst[elm], currTime) + D[identifier]
            else:
                res = f(Rlist, Seq) + T(lst[elm], currTime)
            if v_min > res:
                v_min = res
                Seq = list(Slist)
        R = [[v_min], Seq]
